...
if ($var_array['base']['status_id'] == 2 && $var_array['type'] == 'invoices') {
	...
	$txtStorno =  sprintf($arrPDFConfig['label_cancelled_at'], date('d.m.Y', strtotime($var_array['base']['status_date'])));
...
if ($var_array['type'] != 'deliveries') {
	...
	$pdf->setXY($arrPDFConfig['offsetX'] + $arrPDFConfig['column_unit_offset'] + $arrPDFConfig['column_price_width'] + $arrPDFConfig['column_price_gap'],
		$tmpY);
	$out = ($var_array['base']['curr_id'] == 0) ? $pdf->GetFormatedStandardCurrency($var_array['summ']['rounded_net']) : $pdf->GetFormatedForeignCurrency($var_array['summ']['rounded_curr_net'], $var_array['base']);
	$pdf->Cell($arrPDFConfig['column_price_width'], 0, $out, 0, 0, 'R');
