...
if (!class_exists('TFPDF_TPL')) {
    require_once('tfpdf_tpl.php');
}

/**
 * Class TFPDI
 */
class TFPDI extends TFPDF_TPL
{
    /**
     * FPDI version
     *
     ...
     * @return tfpdi_pdf_parser
     */
    protected function _getPdfParser($filename)
    {
        if (!class_exists('tfpdi_pdf_parser')) {
            require_once('tfpdi_pdf_parser.php');
        }
        return new tfpdi_pdf_parser($filename);
    }
        ...
        foreach($this->parsers AS $filename => $p) {
            ...
            while(($n = key($this->_objStack[$filename])) !== null) {
                try {
                    ...
                    $nObj = array(tpdf_parser::TYPE_OBJECT, tpdf_parser::TYPE_NULL);
                }

                $this->_newobj($this->_objStack[$filename][$n][0]);

                if ($nObj[0] == tpdf_parser::TYPE_STREAM) {
    ...
    protected function _writeValue(&$value)
    {
        ...
        switch ($value[0]) {
            ...
            case tpdf_parser::TYPE_TOKEN:
                $this->_straightOut($value[1] . ' ');
                break;
            case tpdf_parser::TYPE_NUMERIC:
            case tpdf_parser::TYPE_REAL:
            ...
            case tpdf_parser::TYPE_DICTIONARY:
            ...
            case tpdf_parser::TYPE_HEX:
                $this->_straightOut('<' . $value[1] . '>');
                break;

            case tpdf_parser::TYPE_BOOLEAN:
                $this->_straightOut($value[1] ? 'true ' : 'false ');
                break;
            
            case tpdf_parser::TYPE_NULL:
                // The null object.
