...
if (!class_exists('tpdf_parser')) {
    require_once('tpdf_parser.php');
}

/**
 * Class tfpdi_pdf_parser
 */
class tfpdi_pdf_parser extends tpdf_parser
{
    /**
     * Pages
     *
    ...
    protected function _getPageResources($obj)
    {
        ...
        if (isset($obj[1][1]['/Resources'])) {
            ...
            if ($res[0] == tpdf_parser::TYPE_OBJECT)
                return $res[1];
            return $res;
        }
    ...
    protected function _getPageContent($contentRef)
    {
        ...
        if ($contentRef[0] == tpdf_parser::TYPE_OBJREF) {
            $content = $this->resolveObject($contentRef);
            if ($content[1][0] == tpdf_parser::TYPE_ARRAY) {
                $contents = $this->_getPageContent($content[1]);
            } else {
                $contents[] = $content;
            }
        } else if ($contentRef[0] == tpdf_parser::TYPE_ARRAY) {
    ...
    protected function _getPageBox($page, $boxIndex, $k)
    {
        ...
        if (!is_null($box) && $box[0] == tpdf_parser::TYPE_OBJREF) {
            $tmp_box = $this->resolveObject($box);
            $box = $tmp_box[1];
        }
            
        if (!is_null($box) && $box[0] == tpdf_parser::TYPE_ARRAY) {
            $b = $box[1];
            return array(
                'x' => $b[0][1] / $k,
                'y' => $b[1][1] / $k,
