...
class tpdf_parser
{
    ...
    public function __construct($filename)
    {
        ...
        if (!class_exists('tpdf_context')) {
            require_once('tpdf_context.php');
        }
        $this->_c = new tpdf_context($this->_f);
    ...
    protected function _unFilterStream($obj)
    {
        ...
        if (isset($obj[1][1]['/Filter'])) {
            ...
            if ($filter[0] == tpdf_parser::TYPE_OBJREF) {
                $tmpFilter = $this->resolveObject($filter);
                $filter = $tmpFilter[1];
            }

            if ($filter[0] == tpdf_parser::TYPE_TOKEN) {
                $filters[] = $filter;
            } else if ($filter[0] == tpdf_parser::TYPE_ARRAY) {
                $filters = $filter[1];
            }
        }
